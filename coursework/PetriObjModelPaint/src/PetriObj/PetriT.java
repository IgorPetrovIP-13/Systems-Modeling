...
public class PetriT extends PetriMainElement implements Cloneable, Serializable {
   ...
   // private static double timeModeling = Double.MAX_VALUE - 1;
    ...
    public void createInP(ArcIn[] arcs) throws ExceptionInvalidTimeDelay {
        ...
        if (inP.isEmpty() && inPwithInf.isEmpty()) {
    ...
    public boolean condition(PetriP[] pp) {
        for (int i = 0; i < inP.size(); i++) {
            int needed = quantIn.get(i);
            if (needed == -1) {
                int marks = pp[inP.get(i)].getMark();
                if (marks == 0) {
                    return false;
                }
            } else {
                if (pp[inP.get(i)].getMark() < needed) {
                    return false;
                }
            }
        }
        for (int i = 0; i < inPwithInf.size(); i++) {
            int needed = quantInwithInf.get(i);
            if (pp[inPwithInf.get(i)].getMark() < needed) {
                return false;
            }
        }
    ...
    public void actIn(PetriP[] pp, double currentTime) {
        if (this.condition(pp)) {
            for (int i = 0; i < inP.size(); i++) {
                int needed = quantIn.get(i);
                if (needed == -1) {
                    // "Забрать все" маркеры из места
                    needed = pp[inP.get(i)].getMark();
                }
                pp[inP.get(i)].decreaseMark(needed);
            }
            if (buffer == 0) {
